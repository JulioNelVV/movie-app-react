...
function MoviesGrid({ params, viewTitle, defaultTitle }) {
  ...
  const nextPage = () => {
    //Go to the title of the view
    goTo(window.innerHeight - window.innerHeight / 7, 0);
  ...
  if (!isLoading && error === null && data.results.length !== 0) {
    return (
      <>
        <h1 className={style["view-title"]}>
          {viewTitle[currentParam] || defaultTitle}
        </h1>
        ...
        <article className={style["pages"]}>
          <input
            className={`${style["pages-previous"]} ${
              style[`pages-previous--${page === 1 ? "hidden" : "visible"}`]
            }`}
            type="button"
          ...
          <div className={style["pages-indicators"]}>
            <input
              className={style["pages-indicator"]}
              type="button"
            ...
            <input
              ...
              type="button"
              onClick={lastPage}
              value={data.total_pages >= 500 ? "500" : data.total_pages}
            />
          </div>
          <input
            className={`${style["pages-next"]} ${
              style[
                `pages-next--${
                  page === (500 || data.total_pages) ? "hidden" : "visible"
                }`
              ]
            }`}
            type="button"
  ...
  if (!isLoading && (error === null) & (data.results.length === 0)) {
    return (
      <h1>
        Not search results were found for{" "}
        {viewTitle[currentParam] || defaultTitle}
      </h1>
    );
  }
